...
def load_css(file_path):
    with open(file_path) as f:
        st.html(f"<style>{f.read()}</style>") # Load custom CSS for styling the Streamlit app
css_path = ".streamlit/style.css" 
load_css(css_path) # Load custom CSS for styling the Streamlit app
# From Assignment 5
...
with col3:
    date = st.date_input("Enter the Date MM-DD-YYYY")
    #min_value = dt.date(2025,1,17)
...
try:
        ...
        if not df.empty:
            ...
            # converting time column to DateTime and filtering by day
            grouped_df['date'] = grouped_df['date']

            days_list = []

            for d in grouped_df['date']:
                grouped_df['date'] = grouped_df['date'].apply(lambda value: value.split('T')[0])

            grouped_df = grouped_df[grouped_df['date'] == date]
